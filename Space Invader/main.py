...
return_home_surface = font.render("PRESS SPACE TO TRY AGAIN", False, YELLOW)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == SHOOT_LASER and game.current_state == 'playing':
            if random.random() < game.fire_rate_multiplier:
    ...
    elif game.current_state == 'game_complete':
        screen.fill(GREY)
        pygame.draw.rect(screen, YELLOW, (10, 10, 780, 780), 2, 0, 60, 60, 60, 60)
        screen.blit(game_complete_surface,(SCREEN_WIDTH // 2 - game_complete_surface.get_width() // 2, SCREEN_HEIGHT // 2 - 100))
